...
import { strTocdrAtoms } from "helpers/strings";
import balanceConnector from "connectors/balance";

/**
 * FixedcdrInput is a simple numeric input that is assumed to **always** hold
 * a floating point number representing a cdr amount (ie, an amount that
 * will be mutiplied by 1e8 to get to the actual atoms value).
 *
 * This is **not** affected by the global currencyDisplay state.
 *
 * Whenever possible, use the cdrInput component, as it is more flexible and
 * already manages the underlying input value in atoms.
 */
export const FixedcdrInput = ({ currencyDisplay, ...props }) =>
  <FloatInput {...{ ...props, unit: currencyDisplay, maxFracDigits: 8 }} />;

/**
 * cdrInput provides a way to receive commanderu amount inputs. Instead of the usual
 * value/onChange pair, it uses amount/onChangeAmount to track values in commanderu
 ...
 */
@autobind
class cdrInput extends React.Component {
  ...
  changeAmount(value) {
    const { unitDivisor } = this.props;
    const amount = !value ? 0 : strTocdrAtoms(value, unitDivisor);
  ...
  onChange(e) {
    ...
    if (value) {
      ...
      const { unitDivisor } = this.props;
      const amount = strTocdrAtoms(value, unitDivisor);
...
export default balanceConnector(cdrInput);
