...
import { appDataDirectory, getGlobalCfgPath, cdrCfg, getWalletPath, cdrwalletCfg, getcdrRpcCert } from "./main_dev/paths";
...
export function initWalletCfg(testnet, walletPath) {
  ...
  if (!config.has("currency_display")) {
    config.set("currency_display","cdr");
  }
...
export function getWalletCert(certPath) {
  var cert;
  certPath = getcdrRpcCert(certPath);
  try {
...
export function readcdrConfig(configPath, testnet) {
  try {
    if (!fs.existsSync(cdrCfg(configPath))) return;
    const readCfg = ini.parse(Buffer.from(fs.readFileSync(cdrCfg(configPath))).toString());
    let newCfg = {};
...
export function getcdrCert(cdrCertPath) {
  if(cdrCertPath)
    if(fs.existsSync(cdrCertPath))
      return fs.readFileSync(cdrCertPath);

  var certPath = getcdrRpcCert();
...
export function createTempcdrConf() {
  if (!fs.existsSync(cdrCfg(appDataDirectory()))) {
    ...
    fs.writeFileSync(cdrCfg(appDataDirectory()), ini.stringify(cdrConf));
  }
...
export function newWalletConfigCreation(testnet, walletPath) {
  ...
  fs.writeFileSync(cdrwalletCfg(getWalletPath(testnet, walletPath)), ini.stringify(cdrwConf));
}
